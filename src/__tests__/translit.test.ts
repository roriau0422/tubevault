...
describe('latinToCyrillic', () => {
  // Each of these was checked against the live InnerTube search: the romanised
  // form returns 0 results, the Cyrillic form returns 15-20.
  it.each([
  ...
  it('prefers digraphs over their prefixes', () => {
    expect(latinToCyrillic('ch')).toBe('ч'); // not ц + х
    expect(latinToCyrillic('kh')).toBe('х'); // not к + х
