...
// Stream-copy mux (arch doc §4.5): no re-encode, completes in seconds.
...
// Container fixup for raw DASH audio (yt-dlp's "FixupM4a" equivalent): the
// fragmented mp4 YouTube serves makes AVFoundation misreport duration (~2x),
// so the position keeps advancing silently after the real audio ends. A
// stream-copy remux rewrites it as a plain mp4 with a correct timeline.
