...
// One-time migration: audio downloaded before the remux fix is a raw
// fragmented-DASH mp4, which AVFoundation plays with a misreported (~2x)
// duration — the position keeps sliding after the audio really ends.
// Rewrite each file in place with a stream-copy remux.
async function fixupDashAudio(): Promise<void> {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // Leave the original playable file alone; retry on next launch is
      // prevented only after a full pass, so mark done regardless below.
...
// Orphan cleanup: rows without files are deleted; files without rows are removed.
