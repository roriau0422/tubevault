...
// Self-contained InnerTube client (no youtubei.js). YouTube's WEB client is
// SABR-only and gates media behind a poToken we can't mint acceptably on-device,
// so this ports yt-dlp's approach: the ANDROID_VR client still returns plain
// `videoplayback` URLs that download the full file with no poToken/SABR/nsig.
//
// The one hard requirement is a REAL, YouTube-issued visitor_data. A locally
// synthesized one (or none) gets the player request bot-challenged
...
// googlevideo media requests want the issuing client's User-Agent. Exported for
// the RNTP preview path and the download fetches.
...
// The player request occasionally returns LOGIN_REQUIRED when visitor_data has
// gone stale. Refresh it once and retry before giving up.
...
// Recursively collect every compactVideoRenderer in the (deeply nested) search
// response — robust to shelf/section layout changes.
...
export async function searchVideos(query: string): Promise<SearchResult[]> {
  ...
  // ANDROID_VR matches literally — unlike the logged-in web client it does no
  // romanised->Cyrillic expansion, so Mongolian titles are invisible to a Latin
  // query ("javkhlan duunuud" -> searchNoResultsRenderer, "жавхлан дуунууд" ->
  // 17 hits). Only retry when the literal search found nothing, so a query that
  // already works can never be displaced by a transliteration guess.
...
// Search-as-you-type. Separate host from InnerTube, plain GET, no visitor_data
// or poToken. `client=firefox` returns bare JSON — the web player's own
// `client=youtube` wraps it in a `window.google.ac.h(...)` JSONP callback that
// would need stripping. Response shape: [query, suggestions, [], {...}].
...
// Player call yields BOTH metadata and direct-URL adaptive formats in one shot,
// so this is all the download path needs. category is absent on ANDROID_VR
// (no microformat) — acceptable, it is nullable.
