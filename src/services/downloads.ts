...
// googlevideo paces a single unranged GET to ~2x the media bitrate (~32 KB/s
// measured), the way it paces playback streams. Real players and yt-dlp fetch
// these URLs in ranged chunks, which the CDN serves at full speed (~2.7 MB/s
// measured on the same URL). So: sequential Range requests appended to the file.
// The issuing client's User-Agent must match the URL (some CDN edges 403 without it).
const CHUNK_SIZE = 2 * 1024 * 1024; // ~1 chunk/s at observed full speed → smooth progress
...
async function downloadToFile(
  ...
  try {
    ...
    for (;;) {
      ...
      if (total === 0) {
        // 206 responses carry "Content-Range: bytes 0-N/TOTAL".
      ...
      // 200 means the server ignored Range and the body was the whole file.
...
// Exponentially smoothed transfer rate; samples at most every 500ms so bursts
// of progress callbacks don't produce a jittery readout.
class SpeedMeter {
  private lastBytes = 0;
  private lastTime = 0;
  private ema = 0;
...
class DownloadManager {
  ...
  private async runAudio(videoId: string, info: ResolvedVideo): Promise<void> {
    ...
    // file if ffmpeg fails — a playable file beats a failed download.
    try {
