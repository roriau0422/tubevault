...
const STAGGER_LIMIT = 10; // rows past this appear instantly (list recycling)
...
/** Pulsing placeholder rows shown while a search is in flight. */
...
/** Fade + slide entrance, staggered by row index. */
...
/** Bouncing ↓ arrow + percent + mini progress bar while an item downloads. */
...
/** Checkmark that pops in with a spring once the download completes. */
function DoneBadge() {
  const th = useTheme();
...
/** Right side of a result row: download pills → progress → done/retry. */
...
export default function SearchScreen() {
  ...
  // no effect needed to clear them, which the set-state-in-effect rule forbids.
  ...
  // so a slow reply cannot land on top of a newer one.
  useEffect(() => {
