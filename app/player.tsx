...
export default function PlayerModal() {
  ...
  // Which item the controls were dismissed for, rather than a bare boolean:
  // a new track then shows them again for free, with no effect to sync.
  const [hiddenFor, setHiddenFor] = useState<string | null>(null);
  const [touchPing, setTouchPing] = useState(0);
  const [scrubbing, setScrubbing] = useState(false);
  const videoRef = useRef<VideoView>(null);

  // Drag the pinned media down to dismiss. Only the pinned block carries the
  // handlers — on the playlist below, a downward drag has to mean scroll.
  const dragY = useMemo(() => new Animated.Value(0), []);
  const pan = useMemo(
    () =>
      PanResponder.create({
        onMoveShouldSetPanResponder: (_e, g) =>
          // In fullscreen the native controls own the surface — leave them be.
  ...
  // Auto-hide only while playing — paused, the controls stay up. touchPing
  // restarts the timer on a tap; scrubbing suspends it outright, since
  // onTouchStart fires once per gesture and a drag easily outlasts the timer.
  useEffect(() => {
  ...
  // Every stored video is H.264 by construction — pickVideoFormats() only ever
  // selects an avc1 stream so ffmpeg can stream-copy it into mp4.
  const qualityChip = item.height ? `${item.height}p · H.264` : null;
  const sizeChip = item.sizeBytes > 0 ? formatBytes(item.sizeBytes) : null;

  // Everything below the media scrolls, so the playlist can run the full height
  // of the screen while the video stays put — YouTube's watch page.
  const header = (
        ...
        <View style={styles.stage}>
          {isVideo ? (
            <VideoView
              ...
              style={styles.fill}
              // Inline we draw our own controls; fullscreen needs the native
              // ones or there is no way to exit.
          ...
          {controlsShown ? (
            // Plain View, not a Pressable: an ancestor Pressable takes the JS
            // responder on touch-down and steals the drag from the native
            // seek bar. The scrim is a sibling underneath instead, so it can
            // never sit in the slider's touch path. onTouchStart still fires
            // here without claiming the responder.
...
const styles = StyleSheet.create({
  ...
  // Full-bleed 16:9 surface; the container carries no horizontal padding so
  // this needs no width juggling to reach both edges. Artwork is a YouTube
  // thumbnail, so it is 16:9 too and drops straight into the same box.
  stage: {
    width: '100%',
    aspectRatio: 16 / 9,
    backgroundColor: '#000',
  ...
  // ponytail: flat scrim, not a gradient — a gradient needs expo-linear-gradient
  // and reads near-identically over letterboxed video.
  overlay: {
    position: 'absolute',
    top: 0,
    left: 0,
